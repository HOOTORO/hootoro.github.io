...
:   Вопрос автоматической обработки и систематизации непомерно больших данных(всего интерната) по сей день стоит перед передовыми умами науки. Природная неоднородность данных сильно этот процесс усугубляет.
    Для обработки данных {{< anno "публикационного толка" "постов, новостей и т.д." >}}, с помощью ЭВМ, и придумали специальный формат, который в последствии стал стандартом. Нейро-сетей тогда еще не было как можно догадаться.

`На практике`

:   Простая, но изящная, структура формата [(пример)](#структура), способствовала не просто легкой реализации, а вообще {{< anno "автоматизации реализации " "т.е. готовые модули есть во всех профильных фреймворках и инструментах" >}}.
